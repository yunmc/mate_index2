...
onMounted(() => {
    sensors.setOnceProfile({ from_our_platform: 'ponrh.ai'});
    if (getHashUrlParams('ref')) {
        sensors.setOnceProfile({ref_name: 'pornh.ai:' + getHashUrlParams('ref') });
    }else{
        sensors.setOnceProfile({ref_name: 'default_source' });
    }
    _getBanner()
